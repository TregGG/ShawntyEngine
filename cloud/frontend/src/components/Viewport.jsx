...
export default function Viewport({ entities, selectedIndex, onSelect, onEntityMove, camera, onCameraChange, uiMode }) {
  ...
  const render = useCallback(() => {
    ...
    entities.forEach((entity, index) => {
      ...
      // Collider Outline
      if (entity.components?.collider && !isUI) {
        const collider = entity.components.collider
        let cTopLeft = topLeft
        let cRectW = rectW
        let cRectH = rectH
        
        if (!collider.autoBounds) {
           const cOffset = collider.localOffset || [0, 0]
           const cSize = collider.localSize || [1, 1]
           const cHalfW = cSize[0] / 2
           const cHalfH = cSize[1] / 2
           
           const cx = pos[0] + cOffset[0]
           const cy = pos[1] + cOffset[1]
           
           cTopLeft = worldToScreen(cx - cHalfW, cy + cHalfH, canvas)
           const cBotRight = worldToScreen(cx + cHalfW, cy - cHalfH, canvas)
           cRectW = cBotRight.x - cTopLeft.x
           cRectH = cBotRight.y - cTopLeft.y
        }
        
        ctx.strokeStyle = collider.isTrigger ? '#f59e0b' : '#10b981'
        ctx.lineWidth = 1.5
        
        if (collider.isTrigger) ctx.setLineDash([4, 4])
        
        ctx.strokeRect(cTopLeft.x, cTopLeft.y, cRectW, cRectH)
        ctx.fillStyle = collider.isTrigger ? 'rgba(245, 158, 11, 0.15)' : 'rgba(16, 185, 129, 0.15)'
        ctx.fillRect(cTopLeft.x, cTopLeft.y, cRectW, cRectH)
        
        if (collider.isTrigger) ctx.setLineDash([])
      }
  ...
  }, [entities, selectedIndex, camera, worldToScreen, screenToWorld, uiMode])
