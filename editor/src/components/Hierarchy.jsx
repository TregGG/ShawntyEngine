...
export default function Hierarchy({ entities, relationships = [], selectedIndex, onSelect, onDelete, onDuplicate, onAdd, onAddChild, onParent, uiMode }) {
  ...
  const handleDragOver = (e, index) => {
    ...
    if (draggedIndex !== null && draggedIndex !== index && index !== null) {
      if (!isDescendant(index, draggedIndex)) {
  ...
  const toggleExpand = (e, editorId) => {
    e.stopPropagation()
    setExpanded(prev => ({ ...prev, [editorId]: !prev[editorId] }))
  }
  ...
  // Find root entities (those not in any children array)
  ...
  const renderEntity = (index, depth = 0) => {
    ...
    const isExpanded = expanded[entity.editorId] !== false // default expanded
      ...
      <div key={entity.editorId || index}>
        <div
          ...
          style={{ 
            ...
            borderBottom: dropTargetIndex === index ? '2px solid var(--accent)' : 'none',
            background: dropTargetIndex === index ? 'var(--bg-hover)' : '',
        ...
        >
          {hasChildren ? (
            <span
              className={`component-arrow ${isExpanded ? 'open' : ''}`}
              onClick={(e) => toggleExpand(e, entity.editorId)}
            >
              ▶
            </span>
          ) : (
            <span className="component-arrow" style={{ opacity: 0 }}>▶</span>
          )}
          <span className="entity-icon">{icon}</span>
          <span className="entity-name">{entity.name || 'Unnamed'}</span>
          <span className="entity-badges">
            {components.filter(c => c !== 'transform').map(c => (
        ...
        {hasChildren && isExpanded && (
          <div>
            {childIndices.map(childIdx => renderEntity(childIdx, depth + 1))}
          </div>
        )}
      </div>
    )
  ...
  return (
    ...
    >
      <div className="panel-header">
        <span>Hierarchy</span>
        <button className="btn-icon" onClick={onAdd} title="Add Root Entity">+</button>
      </div>
      ...
      {entities.length === 0 ? (
        ...
        <div className="entity-list">
          {rootIndices.map(idx => renderEntity(idx, 0))}
        </div>
      )}
      ...
      {contextMenu && (
        ...
        >
          {onAddChild && (
            <div className="context-menu-item" onClick={() => { onAddChild(contextMenu.index); closeContextMenu() }}>
              👶 Add Child
            </div>
          )}
          <div className="context-menu-item" onClick={() => { onDuplicate(contextMenu.index); closeContextMenu() }}>
            📋 Duplicate
          </div>
          <div className="context-menu-separator" />
          <div className="context-menu-item danger" onClick={() => { onDelete(contextMenu.index); closeContextMenu() }}>
            🗑️ Delete
          </div>
        </div>
      )}
